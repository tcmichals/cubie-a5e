...
project-cubie-a5e/                external tree (BR2_EXTERNAL) — edit here
  ...
  overlays/
    cubie-a5e-flight-stack.dts    DT overlay (SPI/I2C enablement)
  package/
...
docs/                             how-to guides (start here for context)
...
- Overlay: `cubie-a5e-flight-stack.dts` → compiled to `.dtbo` by Buildroot
...
Prebuilt bundle path: `BR2_PACKAGE_TIMVX_DELEGATE_PREBUILT_DIR`

## Known stubs / active TODOs

- `sunxi-galcore` — kernel NPU driver packaging not yet complete
- TIM-VX runtime bundle — must be supplied externally via `BR2_PACKAGE_TIMVX_DELEGATE_PREBUILT_DIR`
...
- `docs/BuildRootHowTo.md` — packages, why they exist, how to add new ones
- `docs/DeviceTreeHow.md` — DT/overlay plain + deep-dive
- `docs/HowToNPU.md` — NPU stack and validation
- `docs/WirelessHowTo.md` — Wi-Fi bring-up
...
- Keep `boot.cmd` and `u-boot-fragment.config` in sync — every command in boot.cmd needs its CONFIG_ in the fragment
- When adding overlays: update `genimage.cfg` + `boot.cmd` + `defconfig` + `DeviceTreeHow.md`
