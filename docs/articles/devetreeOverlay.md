...
* **`uboot-env.txt`** (`project-cubie-a5e/board/radxa/cubie_a5e/uboot-env.txt`)
  * **File Type**: Plain Text Source Template
  * **Generated By**: Developer
  * **Primary Role**: Source template defining baseline U-Boot environment variables
  * **User Editable**: ✅ Yes (edit directly in git repo before compilation)

* **`uboot.env`**
  * **File Type**: 64 KB Binary Blob with CRC32 Checksum
  * **Generated By**: Host tool `mkenvimage`
  * **Primary Role**: Static firmware baseline (baud rates, DRAM addresses, immutable `bootcmd`)
  * **User Editable**: ❌ **No (Do not edit directly — invalidates the CRC32 header)**

* **`boot.cmd`** (`project-cubie-a5e/board/radxa/cubie_a5e/boot.cmd`)
  * **File Type**: Plain Shell Script
  * **Generated By**: Developer
  * **Primary Role**: Source script implementing dynamic multi-overlay discovery and loading logic
  * **User Editable**: ✅ Yes (edit directly in git repo before compilation)

* **`boot.scr`**
  * **File Type**: Binary with 64-byte U-Boot Header
  * **Generated By**: Host tool `mkimage`
  * **Primary Role**: Compiled U-Boot script executed automatically via `source 0x4fc00000`
  * **User Editable**: ❌ No (must recompile from `boot.cmd` using `mkimage`)

* **`config.txt`** (`project-cubie-a5e/board/radxa/cubie_a5e/config.txt`)
  * **File Type**: Plain Text (Raspberry Pi format)
  * **Generated By**: User / Buildroot
  * **Primary Role**: Runtime overlay selection (`dtoverlay=`) and extra kernel command line (`cmdline=`)
  * **User Editable**: ✅ **Yes (Safely editable live on target Linux with `vi` or on PC SD reader)**

* **`uEnv.txt`** (`project-cubie-a5e/board/radxa/cubie_a5e/uEnv.txt`)
  * **File Type**: Plain Text (U-Boot standard format)
  * **Generated By**: User / Buildroot
  * **Primary Role**: Legacy fallback configuration (`overlays=`, `extra_bootargs=`)
  * **User Editable**: ✅ **Yes (Safely editable live on target with `vi`)**
...
* **File Format**:
  * **`uboot.env`**: 64 KB Binary Blob with 4-byte CRC32 header and null padding.
  * **`config.txt`**: Pure Plain ASCII / UTF-8 Text without checksum constraints.
* **Primary Purpose**:
  * **`uboot.env`**: Low-level U-Boot bootstrap plumbing (console baud rate, DRAM mappings, and initial script execution).
  * **`config.txt`**: High-level user configuration (enabling overlays like `dtoverlay=cubie-a5e-flight-stack` and custom `cmdline=` arguments).
* **Safe to Edit with `vi` / Notepad?**:
  * **`uboot.env`**: ❌ **No** (Direct editing invalidates the 4-byte CRC32 header, triggering `*** Bad CRC ***` fallback).
  * **`config.txt`**: ✅ **Yes** (100% human-editable in any text editor without checksum limitations).
* **Where to Edit**:
  * **`uboot.env`**: Requires host `mkenvimage` tool or interactive U-Boot serial console (`setenv` / `saveenv`).
  * **`config.txt`**: Live on target Linux (`vi /boot/config.txt`) or by inserting the SD card into any PC.
* **Ingestion Mechanism**:
  * **`uboot.env`**: Loaded automatically by the U-Boot core binary during board initialization.
  * **`config.txt`**: Imported dynamically into RAM by `boot.cmd` using U-Boot's `env import -t` command.
...
* **Boot Filesystem Architecture**:
  * **Standard Armbian Distribution**: Uses a single monolithic `ext4` root partition where `/boot` resides inside the Linux filesystem.
  * **Our Cubie A5E Buildroot Architecture**: Separates storage into a dedicated 64 MB FAT32 boot partition (`boot.vfat`) plus an `ext4` root partition.
* **Cross-Platform Host Editing**:
  * **Standard Armbian Distribution**: ❌ **Difficult** — Inserting the SD card into Windows or macOS cannot read the `ext4` partition without installing third-party filesystem drivers.
  * **Our Cubie A5E Buildroot Architecture**: ✅ **Instant** — The FAT32 partition automatically mounts as a standard flash drive on Windows, macOS, and Linux PCs out-of-the-box.
* **Configuration Syntax**:
  * **Standard Armbian Distribution**: Uses Armbian-specific variable naming (`armbianEnv.txt` with `overlays=` and `extraargs=`).
  * **Our Cubie A5E Buildroot Architecture**: Adopts the widely-known Raspberry Pi convention (`config.txt` with `dtoverlay=` and `cmdline=`).
* **Ecosystem Compatibility**:
  * **Standard Armbian Distribution**: Locked strictly to the Armbian variable syntax.
  * **Our Cubie A5E Buildroot Architecture**: **Tri-Format Universal Engine** — Natively parses Raspberry Pi `config.txt`, Armbian `armbianEnv.txt`, and legacy `uEnv.txt` seamlessly within a single `boot.cmd`.
