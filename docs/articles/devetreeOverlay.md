...
| :--- | :--- | :--- | :--- | :--- |
| **uboot-env.txt** | Plain Text Source | Developer | Source template for default U-Boot environment | Yes (edit in repo before build) |
| **uboot.env** | 64KB Binary with CRC32 | `mkenvimage` | Static firmware baseline (baud, DRAM addresses, bootcmd) | **NO (Do not edit directly)** |
| **boot.cmd** | Plain Shell Script | Developer | Source script implementing dynamic overlay logic | Yes (edit in repo before build) |
| **boot.scr** | Binary with 64-byte Header | `mkimage` | Compiled U-Boot script executed via `source 0x4fc00000` | Recompile via `mkimage` |
| **config.txt** | Plain Text (Pi-style) | User / Buildroot | Runtime overlay selection (`dtoverlay=`) & kernel cmdline | **Yes (safely editable live with `vi`)** |
| **uEnv.txt** | Plain Text (U-Boot) | User / Buildroot | Legacy fallback configuration (`overlays=`) | **Yes (safely editable live with `vi`)** |
...
| **Safe to Edit with vi?** | **No** (Causes `*** Bad CRC ***` fallback) | **Yes** (100% human-editable in any editor) |
| **Where to Edit** | Host `mkenvimage` or U-Boot serial console | Live on Linux (`/boot/config.txt`) or PC SD reader |
