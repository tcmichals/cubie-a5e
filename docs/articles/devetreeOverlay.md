...
`uboot.env` is **not a text file**. It is a raw binary image compiled by the host tool `mkenvimage` from a text template ([`uboot-env.txt`](../../project-cubie-a5e/board/radxa/cubie_a5e/uboot-env.txt)):
...
1. **Syntax Parsing**: It reads [`uboot-env.txt`](../../project-cubie-a5e/board/radxa/cubie_a5e/uboot-env.txt), stripping comment lines (beginning with `#`) and empty lines.
...
| :--- | :--- | :--- | :--- | :--- |
| **[`uboot-env.txt`](../../project-cubie-a5e/board/radxa/cubie_a5e/uboot-env.txt)** | Plain Text | Developer | Source template for default U-Boot environment | ✅ Edit in repo before build |
| **`uboot.env`** | 64KB Binary with CRC32 | `mkenvimage` | Static firmware baseline (baud, DRAM addresses, bootcmd) | ❌ **Do not edit directly** |
| **[`boot.cmd`](../../project-cubie-a5e/board/radxa/cubie_a5e/boot.cmd)** | Plain Shell Script | Developer | Source script implementing dynamic overlay logic | ✅ Edit in repo before build |
| **`boot.scr`** | Binary with 64-byte Header | `mkimage` | Compiled U-Boot script executed via `source 0x4fc00000` | ❌ Recompile via `mkimage` |
| **[`config.txt`](../../project-cubie-a5e/board/radxa/cubie_a5e/config.txt)** | Plain Text | User / Buildroot | Runtime overlay selection (`dtoverlay=`) & kernel cmdline | ✅ **Safely editable live with `vi`** |
| **[`uEnv.txt`](../../project-cubie-a5e/board/radxa/cubie_a5e/uEnv.txt)** | Plain Text | User / Buildroot | Legacy fallback configuration (`overlays=`) | ✅ **Safely editable live with `vi`** |
...
```
This hands control over to our dynamic boot script ([`boot.cmd`](../../project-cubie-a5e/board/radxa/cubie_a5e/boot.cmd)), which reads `config.txt` directly from the FAT partition into RAM:
...
Here is an example overlay ([`cubie-a5e-uio.dtso`](../../project-cubie-a5e/dts-overlay/allwinner/cubie-a5e-uio.dtso)) that modifies an existing base node and adds memory regions:
