...
* **Dedicated RISC-V RemoteProc Architecture (Co-Processor Coexistence)**: While the physical T527 die includes an auxiliary audio DSP block (for which community projects like [SyterKit](https://github.com/YuzukiHD/SyterKit) and [FreeRTOS-HIFI4-DSP](https://github.com/YuzukiHD/FreeRTOS-HIFI4-DSP) provide custom GCC toolchains), our Linux RemoteProc implementation (`sunxi_rproc.c`) strictly focuses on the **XuanTie E907 RISC-V** co-processor following upstream kernel subsystem separation guidelines (see [DSP Decoupling Rationale](../architecture/dsp_decoupling_rationale.md)). The XuanTie E907 RISC-V core is fully supported by standard, upstream open-source GCC/LLVM toolchains (`riscv-none-elf-*`), making it the most accessible and reproducible real-time coprocessor on the silicon.
...
### 4.2 Co-Processor Coexistence: Why We Explored the DSP (and Why We Standardized on RISC-V)

During the initial hardware bring-up and silicon discovery phase, we **did implement and validate full support for the Cadence HiFi4 DSP**. We proved the DSP clocking tree (`DSP_CLK_REG`), reset state machine (`DSP_RST_REG`), alternate boot vector configuration (`DSP_ALT_RESET_VEC`), fast SIMD vector math routines, and dual-core hardware mailbox concurrency (Port 1 for DSP, Port 3 for RISC-V) on real silicon. This proved that the T527 hardware can operate both co-processors simultaneously without bus or interrupt collisions.

However, once silicon capabilities were proven, we deliberately **decoupled the DSP from `sunxi_rproc.c`** and standardized our active series exclusively on the **XuanTie E907 RISC-V co-processor**.

This architectural decision rests on three clear engineering principles:
...
2. **Toolchain Reproducibility & Open Source Ecosystem**:
   While community projects like [SyterKit](https://github.com/YuzukiHD/SyterKit) and [FreeRTOS-HIFI4-DSP](https://github.com/YuzukiHD/FreeRTOS-HIFI4-DSP) by YuzukiHD provide a custom GCC cross-toolchain (`xtensa-hifi4-elf-gcc`) for the DSP, compiling for the HiFi4 architecture still depends on custom crosstool-NG builds and specialized processor overlays outside standard Linux distributions. In contrast, the XuanTie E907 RISC-V core is fully supported out of the box by standard upstream GNU/LLVM toolchains (`riscv-none-elf-*`) across all major package managers, providing frictionless reproducibility without specialized toolchain builds.
...
> **Historical Archive**: All historical Cadence HiFi4 DSP bare-metal firmware, overlays, and dual-mailbox validation tests are permanently preserved in Git at tag **[`v2.1.0-dsp-archive`](https://github.com/tcmichals/cubie-a5e/tree/v2.1.0-dsp-archive)** (commit `5d97010`) and documented in [`docs/architecture/dsp_decoupling_rationale.md`](../architecture/dsp_decoupling_rationale.md).
